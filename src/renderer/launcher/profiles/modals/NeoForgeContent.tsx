// src/renderer/profiles/modals/NeoForgeContent.tsx
...
interface Props {
  ...
  loaderVersions: (string | { version: string; type?: 'recommended' | 'latest' | 'common' })[]
...
export default function NeoForgeContent({
  minecraftVersions,
  loaderVersions,
  loadingVersions,
  selectedVersion,
    ...
    <div className="h-full flex flex-col">
      <div className="pb-4 border-b border-white/10">
        <div className="flex items-center gap-4 mb-4">
          <h3 className="text-2xl font-bold text-lime-400 whitespace-nowrap">Chọn NeoForge Loader</h3>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-all ${
              showStep === 'loader' ? 'bg-lime-500 text-white' : 'bg-white/10'
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:border-lime-400 text-base"
            />
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pt-4">
        {showStep === 'game' ? (
          <div className="space-y-3">
            {loadingVersions ? (
              <div className="flex flex-col items-center justify-center py-16 text-lime-400">
              ...
              filteredVersions.map((v) => (
                <button
                  ...
                  className={`w-full p-4 bg-white/5 hover:bg-white/10 rounded-xl text-left transition-all flex items-center justify-between ${
                    selectedVersion === v.id ? 'border-2 border-lime-400' : ''
                  }`}
                >
                  <div>
                  ...
                  {selectedVersion === v.id && <Check size={20} className="text-lime-400" />}
                </button>
              ))
            )}
          </div>
          ...
          <div className="space-y-3">
            {loaderVersions.length === 0 ? (
              <p className="text-center text-gray-500 py-16">Không có NeoForge loader cho phiên bản này</p>
            ) : (
                  ...
                  <button
                    ...
                    className={`w-full p-4 rounded-xl text-left transition-all relative overflow-hidden ${
                      selectedLoaderVersion === version
                        ? 'bg-lime-500/30 border-2 border-lime-400'
                        : 'bg-white/5 hover:bg-white/10'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium">NeoForge {version}</p>
                        <p className="text-sm text-gray-400">cho Minecraft {selectedVersion}</p>
                      </div>

                      {(type === 'recommended' || type === 'latest') && (
