// src/renderer/profiles/modals/QuiltContent.tsx
...
interface Props {
  ...
  loaderVersions: (string | QuiltVersionItem)[]
...
export default function QuiltContent({
  minecraftVersions,
  loaderVersions,
  loadingVersions,
  selectedVersion,
    ...
    <div className="h-full flex flex-col">
      <div className="pb-4 border-b border-white/10">
        <div className="flex items-center gap-4 mb-4">
          <h3 className="text-2xl font-bold text-pink-400 whitespace-nowrap">Chọn Quilt Loader</h3>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-all ${
              showStep === 'loader' ? 'bg-pink-500 text-white' : 'bg-white/10'
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:border-pink-400 text-base"
            />
          </div>
        </div>
        ...
        {showStep === 'game' ? (
              ...
              filteredVersions.map((v) => (
                <button
                  ...
                  className={`w-full p-4 bg-white/5 hover:bg-white/10 rounded-xl text-left transition-all flex items-center justify-between ${
                    selectedVersion === v.id ? 'border-2 border-pink-400' : ''
                  }`}
                >
                  <div>
                  ...
                  {selectedVersion === v.id && <Check size={20} className="text-pink-400" />}
                </button>
              ))
            )}
          </div>
          ...
          <div className="space-y-3">
            {loaderVersions.length === 0 ? (
              <p className="text-center text-gray-500 py-16">Không có Quilt loader cho phiên bản này</p>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium">Quilt {version}</p>
