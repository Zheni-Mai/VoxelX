// src/renderer/components/modals/tabs/ResourcePacksTab.tsx
...
export default function ResourcePacksTab({ profile }: Props) {
  ...
  const loadPacks = async () => {
    ...
    setLoading(true)
    try {
      const list = await window.electronAPI.profileAPI.getResourcePackList(profile.gameDirectory)
      setPacks(list || [])
    } catch (err) {
      window.toast.error('Không thể tải danh sách resource pack', 'Lỗi')
      setPacks([])
    } finally {
      setLoading(false)
    }
  ...
  const handleAdd = async () => {
    const result = await window.electronAPI.profileAPI.addResourcePacksFromFolder({ gameDirectory: profile.gameDirectory })
    if (result.success) loadPacks()
  }

  const toggle = async (filename: string, enabled: boolean) => {
    const success = await window.electronAPI.profileAPI.toggleResourcePack({
  ...
  const deletePack = async (filename: string) => {
    if (!confirm(`Xóa resource pack "${filename}"?`)) return
    const success = await window.electronAPI.profileAPI.deleteResourcePack({
      gameDirectory: profile.gameDirectory,
      filename
    })
    if (success) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-6 border-b border-white/10 flex justify-between items-center">
        <h3 className="text-sm font-bold text-white">Danh sách Resource Pack ({packs.length})</h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-2xl font-medium text-gray-400">Chưa có resource pack</p>
          </div>
        ) : (
          packs.map(pack => (
            <motion.div
            ...
            >
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-2xl font-bold text-white">
                RP
              </div>
