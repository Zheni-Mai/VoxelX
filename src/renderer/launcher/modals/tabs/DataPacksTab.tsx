// src/renderer/components/modals/tabs/DataPacksTab.tsx
import { useState, useEffect } from 'react'
import { motion } from 'framer-motion'
import { FolderOpen, Loader2, Trash2, FileArchive } from 'lucide-react'
...
export default function DataPacksTab({ profile }: Props) {
  ...
  const loadPacks = async () => {
    ...
    setLoading(true)
    try {
      const list = await window.electronAPI.profileAPI.getDataPackList(profile.gameDirectory)
      setPacks(list || [])
    } catch (err) {
      window.toast.error('Không thể tải danh sách data pack', 'Lỗi')
      setPacks([])
    } finally {
      setLoading(false)
    }
  ...
  const handleAdd = async () => {
    const result = await window.electronAPI.profileAPI.addDataPacksFromFolder({ gameDirectory: profile.gameDirectory })
    if (result.success) loadPacks()
  }

  const toggle = async (filename: string, enabled: boolean) => {
    const success = await window.electronAPI.profileAPI.toggleDataPack({
  ...
  const deletePack = async (filename: string) => {
    if (!confirm(`Xóa data pack "${filename}"?`)) return
    const success = await window.electronAPI.profileAPI.deleteDataPack({
      gameDirectory: profile.gameDirectory,
      filename
    })
    if (success) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-6 border-b border-white/10 flex justify-between items-center">
        <h3 className="text-sm font-bold text-white">Danh sách Data Pack ({packs.length})</h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-20">
            <FileArchive size={80} className="mx-auto mb-6 text-gray-600" />
            <p className="text-2xl font-medium text-gray-400">Chưa có data pack</p>
          </div>
        ) : (
          packs.map(pack => (
            <motion.div
            ...
            >
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-teal-600 to-emerald-600 flex items-center justify-center text-2xl font-bold text-white">
                DP
              </div>
