...
export function registerProfileHandlers(mainWindow: BrowserWindow) {
    ...
    // Hàm gửi tiến độ AN TOÀN – không crash nếu window đã destroy
    ...
    try {
      ...
      if (ext === '.mrpack') {
        ...
        if (total === 0) {
          safeSend('import-progress', { message: 'Không có mod nào để tải', percent: 100 })
        } else {
          safeSend('import-progress', {
            message: 'Đang tải mods từ Modrinth...',
            percent: 5
          })
          ...
          for (const file of files) {
            ...
            try {
              ...
              downloaded++
              const percent = Math.round((downloaded / total) * 90) + 5
        ...
        safeSend('import-progress', { message: 'Hoàn tất!', percent: 100 })
          ...
          const profilesPath = getProfilesFilePath(appDataPath)
          let profiles: Profile[] = []
          try {
    ...
    ipcMain.handle('modrinth:download-file', async (_event, fileUrl: string, fileName: string, profilePath: string, metadata?: any) => {
      try {
        ...
        const modpackPath = path.join(profilePath, 'modpack.json')
        let data = { mods: [] as any[] }
        try {
          if (await access(modpackPath).then(() => true).catch(() => false)) {
            data = JSON.parse(await readFile(modpackPath, 'utf-8'))
        ...
        console.error('Download mod failed:', err)
        return false
      }
    })
    ...
    ipcMain.handle('profile:recognizeMod', async (_event, { filePath }: { filePath: string }) => {
      try {
        ...
        // Tìm phiên bản mới nhất phù hợp
        const version = data
    ...
    ipcMain.handle('modrinth:search', async (_event, query: string, gameVersion?: string, loader?: 'fabric' | 'forge' | 'quilt' | 'all', limit = 100, offset = 0) => {
      try {
    ...
    for (const filePath of result.filePaths) {
      try {
        ...
        // TỰ ĐỘNG NHẬN DIỆN MOD TỪ MODRINTH
